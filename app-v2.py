...
def initialize_tts_engine():
    ...
    if 'pyttsx3_engine' not in st.session_state or st.session_state.pyttsx3_engine is None:
        try:
            ...
            print(f"Error initializing pyttsx3: {e}. Voice output may not be available.") # Keep as print for server-side log
...
def initialize_groq_client():
    ...
    if 'groq_client' not in st.session_state or st.session_state.groq_client is None:
        if not GROQ_API_KEY:
            print("GROQ_API_KEY is not set. Cannot connect to LLM.") # Keep as print
        ...
        try:
            ...
            print(f"Error initializing Groq client: {e}") # Keep as print
            st.session_state.groq_client = None
...
def speak_directly(text, lang='en'):
    ...
    if 'pyttsx3_engine_initialized' in st.session_state and st.session_state.pyttsx3_engine_initialized and st.session_state.pyttsx3_engine:
        try:
            ...
            print(f"Error in pyttsx3 direct speech: {e}") # Keep as print
            return False
    else:
        print("pyttsx3 engine not initialized. Cannot generate audio.") # Keep as print
        return False
...
def get_groq_response_st(user_prompt):
    ...
    try:
        ...
        print(f"Error communicating with Groq API: {e}") # Keep as print
...
def listen_for_voice_st(lang='en', timeout=5, phrase_time_limit=10):
    ...
    with microphone as source:
        # st.info("Adjusting for ambient noise... Please wait.") # Remove frontend log
        try:
            recognizer.adjust_for_ambient_noise(source, duration=1)
            # st.info(f"Listening for input in {lang}... (Speak now)") # Remove frontend log
            audio = recognizer.listen(source, timeout=timeout, phrase_time_limit=phrase_time_limit)
            # st.info("Recognizing...") # Remove frontend log
            text = recognizer.recognize_google(audio, language=lang)
            # st.success(f"You said: {text}") # Remove frontend log
            return text
        except sr.WaitTimeoutError:
            # st.warning("No speech detected within the time limit.") # Remove frontend log
            return None
        except sr.UnknownValueError:
            print("Speech Recognition could not understand audio.") # Keep as print
            return None
        except sr.RequestError as e:
            print(f"Could not request results from Speech Recognition service; {e}") # Keep as print
            return None
        except Exception as e:
            print(f"An unexpected error occurred during speech recognition: {e}") # Keep as print
            return None
...
def run_kisaanvaani_app():
    ...
    if st.session_state.get('conversation_active', False):
        if st.sidebar.button("End Conversation", key="toggle_conversation_btn", use_container_width=True):
            st.session_state.conversation_active = False
            st.session_state.trigger_listen = False
            # st.info("Conversation ended.") # Remove frontend log
            st.rerun()
    else:
        if st.sidebar.button("Start Conversation", key="toggle_conversation_btn", use_container_width=True):
            ...
            # st.info("Conversation started. I'm listening...") # Remove frontend log
            st.rerun()
    ...
    if st.session_state.get('conversation_active') and st.session_state.get('trigger_listen'):
        ...
        with st.spinner("Listening for your query..."):
            ...
            if voice_input_text:
                ...
                # st.rerun() # REMOVED: Let the script complete. If no query was processed,
                # Streamlit's natural rerun or a later explicit rerun will pick up trigger_listen.
    ...
    if query_to_process_now:
        ...
        if st.session_state.get('pyttsx3_engine_initialized', False) and st.session_state.pyttsx3_engine:
            ...
            print("TTS engine not initialized, skipping speech for AI response.") # Keep as print
        elif not st.session_state.pyttsx3_engine:
            print("TTS engine object is None, skipping speech for AI response.") # Keep as print
    ...
    if not st.session_state.chat_history and st.session_state.get('services_initialized'):
        ...
        if st.session_state.get('pyttsx3_engine_initialized', False) and st.session_state.pyttsx3_engine:
            ...
            print("TTS engine not initialized, skipping greeting speech.") # Keep as print
        elif not st.session_state.pyttsx3_engine:
            print("TTS engine object is None, skipping greeting speech.") # Keep as print
