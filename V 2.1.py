...
class ToolSystem:
    ...
    def get_current_date(self, **kwargs):
        ...
        try:
            ...
            return "I encountered an error getting the current date and time."
    ...
    def get_weather_forecast(
        ...
        """
        Get weather data using WeatherAPI.com, optimized for enterprise production use.

        Features: LRU Caching, Thread-Safety, Circuit Breaker, Retries with Jitter,
        Granular Error Handling, Observability Hooks, and Unicode Normalization.
        
        Note on Production Use:
            - This function should be augmented with real metrics hooks (e.g., Prometheus).
            - It requires a comprehensive unit test suite using mocking libraries.
        """
        # 1. Thread-safe, one-time initialization of state and config
        if not hasattr(self, '_weather_cache_lock'):
            with self.__class__._weather_init_lock:
                if not hasattr(self, '_weather_cache_lock'):
                    ...
                    self.CIRCUIT_BREAKER_TIMEOUT_SECONDS = 180
                    # State Variables and Utilities
                    self._weather_requests_session = requests.Session()
                    self._weather_cache = OrderedDict()
                    self._weather_circuit_breaker = {'failures': 0, 'open_until': 0}
                    self._metrics = MetricsEmitter() # Placeholder for real metrics client
                    # Locks for granular, thread-safe state modification
                    self._weather_cache_lock = threading.Lock()
                    self._weather_circuit_lock = threading.Lock()

        # 2. Circuit Breaker and Cache Check
        with self._weather_circuit_lock:
            if time.time() < self._weather_circuit_breaker['open_until']:
                self._metrics.inc_counter('weather.circuit_breaker.opened')
                return {"success": False, "message": "The weather service is temporarily down, please try again in a few minutes.", "error": "Circuit breaker is open.", "code": "CIRCUIT_OPEN"}

        with self._weather_cache_lock:
            # Normalize location for robust caching
            loc_key = unicodedata.normalize("NFC", location.strip().lower())
            if not loc_key: loc_key = "auto:ip"
            cache_key = (loc_key, include_forecast, include_air_quality, lang)
            if cache_key in self._weather_cache:
                cached_data, timestamp = self._weather_cache[cache_key]
                if time.time() - timestamp < self.CACHE_TTL_SECONDS:
                    self._weather_cache.move_to_end(cache_key)
                    tool_logger.info("Returning cached weather data.", extra={"tool": "weather", "location": loc_key})
                    self._metrics.inc_counter('weather.cache.hit')
                    return cached_data
        ...
        if include_air_quality: params["aqi"] = "yes"
        if include_forecast: params.update({"days": 3, "alerts": "yes"})
        if lang: params["lang"] = lang

        # 4. API Call with Retries and Jitter
        for attempt in range(self.MAX_RETRIES):
            ...
            try:
                ...
                if response.status_code == 200:
                    with self._weather_circuit_lock: self._weather_circuit_breaker['failures'] = 0
                    data = response.json()
                    ...
                    result = {"success": True, "message": summary, "data": data}
                    with self._weather_cache_lock:
                        self._weather_cache[cache_key] = (result, time.time())
                        if len(self._weather_cache) > self.CACHE_SIZE:
                            evicted_key, _ = self._weather_cache.popitem(last=False)
                            tool_logger.info("Weather cache evicted oldest entry.", extra={"tool": "weather", "evicted_key": evicted_key})
                            self._metrics.inc_counter('weather.cache.evicted')
                    return result

                elif response.status_code == 429:
                    self._metrics.inc_counter('weather.error.rate_limit')
                    return {"success": False, "message": "Too many requests right now. Please wait a moment.", "error": "Rate limit exceeded.", "code": "RATE_LIMIT_EXCEEDED"}
                
                elif response.status_code == 403:
                    try: error_code = response.json().get("error", {}).get("code")
                    except ValueError: error_code = None
                    self._metrics.inc_counter('weather.error.forbidden')
                    msg = "Your plan may not support this feature." if error_code == 2008 else "Access to the weather service was denied."
                    return {"success": False, "message": msg, "error": msg, "code": "FORBIDDEN"}
                
                elif response.status_code >= 500:
                    with self._weather_circuit_lock:
                        self._weather_circuit_breaker['failures'] += 1
                        if self._weather_circuit_breaker['failures'] >= self.CIRCUIT_BREAKER_THRESHOLD:
                            self._weather_circuit_breaker['open_until'] = time.time() + self.CIRCUIT_BREAKER_TIMEOUT_SECONDS
                            self._metrics.inc_counter('weather.circuit_breaker.tripped')
                
                else: # Handle other 4xx client errors
                    try: error_msg = response.json().get("error", {}).get("message")
                    except ValueError: error_msg = response.text[:200]
                    return {"success": False, "message": error_msg or "An unknown error occurred.", "error": error_msg, "code": f"HTTP_{response.status_code}"}

            except (requests.exceptions.RequestException, ValueError) as e:
                tool_logger.warning(f"Network or JSON error on attempt {attempt + 1}: {e}")

            self._metrics.inc_counter('weather.api.retry')
            if attempt < self.MAX_RETRIES - 1:
                time.sleep(self.BACKOFF_FACTOR * (2 ** attempt) + random.uniform(0, 0.5))

        final_msg = "The weather service is currently unresponsive after multiple attempts."
        tool_logger.error(final_msg, extra={"tool": "weather", "location": loc_key, "code": "SERVICE_UNAVAILABLE"})
        self._metrics.inc_counter('weather.error.service_unavailable')
        return {"success": False, "message": "I'm unable to reach the weather service right now.", "error": final_msg, "code": "SERVICE_UNAVAILABLE"}

    # Dummy class for demonstration
    # class MetricsEmitter:
    #      def inc_counter(self, name: str): pass
    #      def observe_timer(self, name: str, duration: float): pass    
    ...
    def get_news_summary(self, topic: str = None, **kwargs):
        """Get a summary of current news, optionally filtered by topic."""
        try:
            if not TAVILY_API_KEY:
                return "News summary service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                tool_logger.info("Tavily client initialized for get_news_summary.")
            ...
            if response.get("answer"):
                return response["answer"]
            return "Could not retrieve news summary."
        except Exception as e:
    ...
    def get_definition(self, term: str, **kwargs):
        ...
        try:
            if not TAVILY_API_KEY:
                return "Definition service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for get_definition.")
    ...
    def get_biography(self, person: str, **kwargs):
        """Get a short biography of a person."""
        try:
            if not TAVILY_API_KEY:
                return "Biography service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for get_biography.")
            ...
            if response.get("answer"):
                return response["answer"]
            return f"Could not find a biography for {person}."
        except Exception as e:
            tool_logger.error(f"Biography Error: {e}")
            return f"Error getting biography: {str(e)}"

    def get_sports_update(self, sport: str = None, **kwargs):
        """Get updates on sports news."""
        try:
            if not TAVILY_API_KEY:
                return "Sports update service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for get_sports_update.")
            ...
            if response.get("answer"):
                return response["answer"]
            return "Could not retrieve sports updates."
        except Exception as e:
            tool_logger.error(f"Sports Update Error: {e}")
            return f"Error getting sports update: {str(e)}"

    def get_movie_info(self, title: str, **kwargs):
        """Get information about a movie."""
        try:
            if not TAVILY_API_KEY:
                return "Movie info service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for get_movie_info.")
            ...
            if response.get("answer"):
                return response["answer"]
            return f"Could not find information for movie {title}."
        except Exception as e:
            tool_logger.error(f"Movie Info Error: {e}")
            return f"Error getting movie information: {str(e)}"

    def get_tech_news(self, **kwargs):
        """Get the latest technology news."""
        try:
            if not TAVILY_API_KEY:
                return "Tech news service is not configured. Please check API settings."
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for get_tech_news.")
            ...
            if response.get("answer"):
                return response["answer"]
            return "Could not retrieve technology news."
        except Exception as e:
    ...
    def web_search(self, query: str, **kwargs):
        ...
        try:
            if not TAVILY_API_KEY:
                tool_logger.warning("TAVILY_API_KEY is not set. Web search service is not configured.")
            ...
            if not self.tavily_client:
                ...
                os.environ['REQUESTS_CA_BUNDLE'] = certifi.where()
                self.tavily_client = TavilyClient(api_key=TAVILY_API_KEY)
                tool_logger.info("Tavily client initialized for web_search.")
            ...
            # Format results if no direct answer
            results = []
            ...
            if "TLS" in str(e) or "SSL" in str(e) or "certificate" in str(e).lower():
                return "I'm having trouble with the secure connection. Please try again in a moment."
            elif "API" in str(e) or "key" in str(e).lower():
                return "The search service is not properly configured. Please check API settings."
            else:
...
3. No tool needed: Use this option when the user's query is purely conversational, a greeting, or an acknowledgement that doesn't require external information or computation.
...
6. Extract parameters precisely from the user's query and context. For the `get_weather_forecast` tool specifically, if the query mentions a future time (like 'tomorrow', 'in 2 days', or 'forecast'), you MUST set the `include_forecast` parameter to `True`.
7. Your output MUST be a JSON object with:
   ...
   - context_used: List of context elements that influenced your decision"""
        ...
        if chosen_tool and chosen_tool != "None":
            if chosen_tool in ["web_search", "get_weather_forecast", "get_mandi_prices", "get_farming_advice", "get_crop_calendar", "get_fertilizer_info", "get_pest_control", "get_irrigation_advice"]:
            ...
            if tool_response["success"]:
                ...
                # Intelligently handle both new dictionaries and old strings
                if isinstance(tool_result, dict) and 'message' in tool_result:
                    tool_output = tool_result['message']
                else:
                    tool_output = str(tool_result)

                if is_farming_query and chosen_tool in ["get_mandi_prices", "get_farming_advice", "get_crop_calendar", "get_fertilizer_info", "get_pest_control", "get_irrigation_advice"]:
                    chat_context_instance.update_farming_context(chosen_tool, tool_result)
            
            else: # This block now handles all failure cases cleanly
