...
class ToolSystem:
    ...
    def get_weather_forecast(self, location: str = "current location", **kwargs):
        """Get weather forecast for an Indian city using IndianAPI Weather API, with robust error handling and WeatherAPI fallback."""
        INDIANAPI_KEY = "sk-live-IwV6RGZ7IkMsMEpBxrghlo7NpbdkFkdGC7WDcTg0"
        WEATHERAPI_KEY = "76fd8c1157635b5a06749e492bd44be2"
        if not location or location.strip().lower() in ["current location", "", None]:
            return {
                "success": False,
                "error": "Location not provided.",
                "message": "I need your city to provide the weather. Could you please tell me your location?",
                "code": "MISSING_LOCATION"
            }
        url = f"https://weather.indianapi.in/india/weather?city={location}"
        headers = {"x-api-key": INDIANAPI_KEY}
        tool_logger.warning("Disabling SSL verification for IndianAPI Weather API call. This is insecure and should only be used in trusted environments.")
        tool_logger.debug(f"Requesting IndianAPI Weather: URL={url}, Headers={headers}")
        try:
            response = requests.get(url, headers=headers, timeout=10, verify=False)
            tool_logger.debug(f"IndianAPI Weather Raw Response: HTTP {response.status_code} - {response.text}")
            # Check HTTP status
            if response.status_code == 404:
                indianapi_error = {
                    "success": False,
                    "error": "City not found.",
                    "message": f"I couldn't find weather information for '{location}'. Please check the city name or try another city.",
                    "code": "CITY_NOT_FOUND"
                }
                raise Exception("IndianAPI city not found")
            if response.status_code != 200:
                indianapi_error = {
                    "success": False,
                    "error": f"HTTP {response.status_code}",
                    "message": f"Weather service error (code {response.status_code}). Please try again later.",
                    "code": "HTTP_ERROR"
                }
                raise Exception(f"IndianAPI HTTP error {response.status_code}")
            try:
                data = response.json()
            except ValueError:
                tool_logger.error(f"IndianAPI Weather JSON decode error: {response.text}")
                indianapi_error = {
                    "success": False,
                    "error": "Invalid response format.",
                    "message": "Received an unexpected response from the weather service. Please try again later.",
                    "code": "INVALID_JSON"
                }
                raise Exception("IndianAPI invalid JSON")
            # Check for API-specific error in JSON
            if not data or not isinstance(data, dict):
                tool_logger.error(f"IndianAPI Weather: Unexpected data structure: {data}")
                indianapi_error = {
                    "success": False,
                    "error": "Malformed response.",
                    "message": "Weather service returned an unexpected response. Please try again later.",
                    "code": "MALFORMED_RESPONSE"
                }
                raise Exception("IndianAPI malformed response")
            if 'detail' in data and isinstance(data['detail'], str):
                indianapi_error = {
                    "success": False,
                    "error": data['detail'],
                    "message": f"Weather service error: {data['detail']}. Please check the city name or try again later.",
                    "code": "API_ERROR"
                }
                raise Exception(f"IndianAPI API error: {data['detail']}")
            if 'weather' not in data or 'current' not in data['weather']:
                tool_logger.error(f"IndianAPI Weather: Missing expected keys in response: {data}")
                indianapi_error = {
                    "success": False,
                    "error": "No weather data.",
                    "message": f"No weather data found for '{location}'. Please check the city name or try another city.",
                    "code": "NO_WEATHER_DATA"
                }
                raise Exception("IndianAPI no weather data")
            weather = data['weather']['current']
            city = data.get('city', location)
            temp_max = weather.get('temperature', {}).get('max', {}).get('value')
            temp_min = weather.get('temperature', {}).get('min', {}).get('value')
            humidity_morning = weather.get('humidity', {}).get('morning')
            humidity_evening = weather.get('humidity', {}).get('evening')
            rainfall = weather.get('rainfall')
            summary_parts = [f"Weather in {city}:"]
            if temp_max is not None:
                summary_parts.append(f"Max Temp: {temp_max}°C")
            if temp_min is not None:
                summary_parts.append(f"Min Temp: {temp_min}°C")
            if humidity_morning is not None:
                summary_parts.append(f"Humidity (Morning): {humidity_morning}%")
            if humidity_evening is not None:
                summary_parts.append(f"Humidity (Evening): {humidity_evening}%")
            if rainfall is not None:
                summary_parts.append(f"Rainfall: {rainfall} mm")
            ...
            return {
                ...
                "data": {
                    ...
                    "temp_max": temp_max,
                    "temp_min": temp_min,
                    "humidity_morning": humidity_morning,
                    "humidity_evening": humidity_evening,
                    "rainfall": rainfall,
                    "summary": summary
                },
                "message": summary
            ...
            tool_logger.warning(f"IndianAPI failed for {location}: {e}. Attempting WeatherAPI fallback.")
            # WeatherAPI fallback
            try:
                if not WEATHERAPI_KEY:
                    return {
                        "success": False,
                        "error": "No fallback API key configured.",
                        "message": "Both IndianAPI and fallback WeatherAPI are unavailable.",
                        "code": "NO_FALLBACK_API_KEY"
                    }
                url = f"http://api.weatherapi.com/v1/current.json?key={WEATHERAPI_KEY}&q={location}&aqi=no"
                tool_logger.debug(f"Requesting WeatherAPI fallback: URL={url}")
                response = requests.get(url, timeout=10)
                tool_logger.debug(f"WeatherAPI Raw Response: HTTP {response.status_code} - {response.text}")
                if response.status_code != 200:
                    return {
                        "success": False,
                        "error": f"WeatherAPI HTTP {response.status_code}",
                        "message": f"Both IndianAPI and WeatherAPI failed (WeatherAPI code {response.status_code}). Please try again later.",
                        "code": "WEATHERAPI_HTTP_ERROR"
                    }
                data = response.json()
                if 'error' in data:
                    tool_logger.error(f"WeatherAPI Error: {data['error']}")
                    return {
                        "success": False,
                        "error": data['error'],
                        "message": f"Both IndianAPI and WeatherAPI failed: {data['error'].get('message', 'Unknown error')}",
                        "code": "WEATHERAPI_API_ERROR"
                    }
                ...
                summary = (f"[Fallback] Weather in {location_name}, {region}, {country}: {condition}. "
                ...
                tool_logger.error(f"WeatherAPI fallback failed for {location}: {e2}")
                # Return the best error message from both sources
                return indianapi_error if 'indianapi_error' in locals() else {
                    "success": False,
                    "error": str(e2),
                    "message": "Both IndianAPI and WeatherAPI failed. Please try again later.",
                    "code": "BOTH_APIS_FAILED"
                }
...
6. For tools requiring parameters, extract them precisely from the user's query and the current context. Be specific and avoid passing the entire query if only a part is relevant. For example, if the query is "What is the weather in Delhi?", extract `location: "Delhi"`.
7. Your output MUST be a JSON object with:
   ...
   - context_used: List of context elements that influenced your decision"""
        ...
        if chosen_tool and chosen_tool != "None":  # Only execute tool if a specific tool is chosen
            ...
            if tool_response["success"]:
                ...
                if is_farming_query and chosen_tool in ["get_mandi_prices", "get_farming_advice", "get_crop_calendar", "get_fertilizer_info", "get_pest_control", "get_irrigation_advice"]:
                    chat_context_instance.update_farming_context(chosen_tool, tool_output)
            else:
                ...
                if confidence < 0.7 and alternative_tools:
                    ...
                    for alt_tool in alternative_tools:
                        ...
                        alt_response = tool_system_instance.execute_tool(alt_tool, params)
            ...
            tool_output = "" # No tool output in this case
