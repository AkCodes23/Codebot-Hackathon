...
# gTTS is used on-the-fly and does not require explicit global initialization here.

def speak(text, lang=LANGUAGE):
    """Converts text to speech using gTTS and playsound."""
    try:
        print("Attempting to speak with gTTS...")
        tts_obj = gTTS(text=text, lang=lang, slow=False)
        # Create temp file in a more reliable way for Windows
        temp_dir = tempfile.gettempdir()
        temp_file = os.path.join(temp_dir, f"kisaanvaani_{int(time.time())}.mp3")
        ...
        # Fallback to console print if gTTS fails
...
def main():
    ...
    # TTS (gTTS) is initialized on-the-fly within the speak function.
