...
def _speak_thread_target(text, lang):
    try:
        ...
        # Get language for gTTS (fallback to English)
        gtts_lang = gtts_lang_mapping.get(lang, 'en')
        
        voice_logger.info(f"TTS Thread: Generating Google TTS for '{text[:70]}...' (Lang: {lang} -> {gtts_lang})")
        
        # Create gTTS object with proper language
        ...
        voice_logger.info(f"TTS Thread: Audio saved for language {gtts_lang}")
        ...
        try:
            ...
            if os.name == 'nt':  # Windows
                try:
                    ...
                    voice_logger.error(f"TTS Thread: Windows audio player failed: {e}")
            elif os.name == 'posix':  # Linux/Mac
                # Try different audio players
                for player in ['mpv', 'vlc', 'ffplay', 'paplay']:
                    try:
                        result = subprocess.run([player, temp_file_path], 
                                              stdout=subprocess.DEVNULL, 
                                              stderr=subprocess.DEVNULL,
                                              timeout=20)
                        if result.returncode == 0:
                            audio_played = True
                            voice_logger.info(f"TTS Thread: Audio played using {player}")
                            break
                    except (FileNotFoundError, subprocess.TimeoutExpired):
                        continue
            
            if not audio_played:
                voice_logger.error("TTS Thread: No audio player worked")
                return
            
            voice_logger.info(f"TTS Thread: Finished speaking: '{text[:70]}...' in {gtts_lang}")
            
            # Add buffer time to ensure audio completely stops before microphone starts
            time.sleep(2.5)  # 2.5 second buffer to prevent feedback
            voice_logger.info(f"TTS Thread: Buffer time completed, TTS fully done")
        ...
        voice_logger.error("TTS Thread: gTTS not installed. Please install with: pip install gtts")
    except Exception as e:
        voice_logger.error(f"TTS Thread: ERROR during Google TTS for '{text[:70]}...': {e}")
...
def speak_non_blocking(text, lang='en'):
    ...
    try:
        ...
        estimated_duration = (num_words / WORDS_PER_SECOND) + 3.0  # Added 2.5s buffer + 0.5s safety
        sleep_duration = max(MIN_SLEEP_AFTER_TTS, min(estimated_duration, MAX_SLEEP_AFTER_TTS))
        
        logger.info(f"Main: Main thread will sleep for {sleep_duration:.2f}s to allow TTS thread to complete with buffer ('{text[:30]}...').")
    ...
    def information_retrieval(self, query: str, **kwargs):
        ...
        try:
            if not self.tavily_client:
                return {"success": False, "answer": None, "error": "Information retrieval service is not configured.", "suggestion": "Please check API settings.", "sources": []}
            ...
            # Prioritize .gov.in or official sources for government scheme queries
            gov_keywords = ["government scheme", "pm kisan", "subsidy", "yojana", "govt scheme", "dbt", "farmer scheme", "pradhan mantri", "fasal bima", "crop insurance", "kisan credit"]
            if any(kw in query.lower() for kw in gov_keywords):
                gov_sources = [s for s in sources if ".gov.in" in s["url"] or ".nic.in" in s["url"]]
                non_gov_sources = [s for s in sources if s not in gov_sources]
                sources = gov_sources + non_gov_sources
            if response.get("answer"):
                return {"success": True, "answer": response["answer"], "error": None, "suggestion": None, "sources": sources}
            if sources:
                joined = "\n\n".join([f"{s['title']}\n{s['url']}" for s in sources])
                return {"success": True, "answer": joined, "error": None, "suggestion": None, "sources": sources}
            return {"success": False, "answer": None, "error": "No relevant information found.", "suggestion": "Try rephrasing your question.", "sources": []}
        except Exception as e:
            return {"success": False, "answer": None, "error": str(e), "suggestion": "Please try again later.", "sources": []}
...
def handle_user_input(user_input_text, input_type="text"):
    ...
    # Always attach sources if present, even for LLM-synthesized answers
    if agent_output.get("sources") and agent_output["sources"]:
        assistant_message["sources"] = agent_output["sources"]
    ...
    st.rerun()
    save_current_chat()  # Persist after every message
...
def display_chat_history():
    for message in st.session_state.chat_history:
        with st.chat_message(message["role"]):
            ...
            # --- Show sources as clickable buttons if present and toggle is ON ---
            if st.session_state.get("show_sources_toggle", True) and message.get("sources"):
                st.markdown("<div style='margin-top: 0.5em; margin-bottom: 0.5em;'><b>Sources:</b></div>", unsafe_allow_html=True)
                for source in message["sources"]:
                    st.markdown(f"""
                        ...
                        {source['title'] if source['title'] else source['url']}
                        </a>
                    """, unsafe_allow_html=True)
                # Debug print of sources
                st.markdown(f"<div style='color:#888;font-size:0.8em;'>[DEBUG] Sources: {message['sources']}</div>", unsafe_allow_html=True)
...
def run_kisaanvaani_app():
    ...
    st.sidebar.toggle("Show Sources for Answers", key="show_sources_toggle", value=True, help="Show clickable source links below answers when available.")
    ...
    def save_current_chat():
        # Save current chat to chats list
        if st.session_state.get('chat_history'):
            ...
            chat_obj = {
                'id': chat_id or f"chat_{len(st.session_state.chats)+1}",
                'title': title,
            ...
            for i, c in enumerate(st.session_state.chats):
                if c['id'] == chat_obj['id']:
                    st.session_state.chats[i] = chat_obj
                    replaced = True
                    break
            if not replaced:
                st.session_state.chats.append(chat_obj)
            save_chats_to_file()

    def start_new_chat():
        save_current_chat()
        st.session_state.chat_history = []
        st.session_state.current_chat_id = f"chat_{len(st.session_state.chats)+1}"
        st.session_state.conversation_active = False
        st.session_state.should_listen = False
        st.session_state.listening_state = False
        st.session_state.has_greeted_initial = False
    ...
    def sidebar_chat_manager():
        ...
        if st.sidebar.button("➕ New Chat", use_container_width=True, key="new_chat_btn"):
            start_new_chat()
        search_term = st.sidebar.text_input("🔍 Search chats", "", key="search_chats")
        filtered_chats = st.session_state.chats
        if search_term:
            filtered_chats = [c for c in st.session_state.chats if search_term.lower() in c['title'].lower()]
        for chat in filtered_chats[::-1]:  # Show most recent first
            is_editing = st.session_state.editing_chat_id == chat['id']
            label = chat['title']
            if st.session_state.current_chat_id == chat['id']:
                label = f"👉 {label}"
            col1, col2 = st.sidebar.columns([0.8, 0.2], gap="small")
            with col1:
                if is_editing:
                    new_title = st.text_input("Edit chat title", st.session_state.editing_chat_title or chat['title'], key=f"edit_title_{chat['id']}")
                    st.session_state.editing_chat_title = new_title
                else:
                    if st.sidebar.button(label, key=f"chat_{chat['id']}"):
                        load_chat(chat['id'])
            with col2:
                if is_editing:
                    if st.button("💾", key=f"save_rename_{chat['id']}"):
                        rename_chat(chat['id'], st.session_state.editing_chat_title.strip() or chat['title'])
                    if st.button("❌", key=f"cancel_rename_{chat['id']}"):
                        st.session_state.editing_chat_id = None
                        st.session_state.editing_chat_title = ""
                        st.rerun()
                else:
                    if st.button("✏️", key=f"edit_rename_{chat['id']}"):
                        st.session_state.editing_chat_id = chat['id']
                        st.session_state.editing_chat_title = chat['title']
                        st.rerun()
   ...
   - context_used: List of context elements that influenced your decision"""
        ...
        if chosen_tool and chosen_tool != "None":
            ...
            if tool_response["success"]:
                ...
                if "sources" in tool_response and tool_response["sources"]:
...
9. ALWAYS end your response by asking a gentle, open-ended follow-up question."""
        ...
        return {"response": KNOWN_FALLBACK_RESPONSES[1], "sources": []}
...
def ensure_data_dir():
    try:
        pathlib.Path(CHAT_DATA_DIR).mkdir(parents=True, exist_ok=True)
    except Exception as e:
...
def save_chats_to_file():
    ...
    try:
        ...
        logger.info(f"Chats saved to {CHAT_DATA_FILE}")
    except Exception as e:
...
def load_chats_from_file():
    ...
    try:
        with open(CHAT_DATA_FILE, "r", encoding="utf-8") as f:
            chats = json.load(f)
        logger.info(f"Loaded {len(chats)} chats from {CHAT_DATA_FILE}")
        return chats
    except Exception as e:
